...
import { generateHazard, applyHazardToPlayer, applyHazardToEnemy, tickHazard, removeHazardEffects } from "@/lib/hazard-system"
import {
...
import { getInteractionsForEntity, getAvailableInteractions } from "@/lib/environmental-system"
...
export function DungeonGame() {
  ...
  const [currentNarrative, setCurrentNarrative] = useState<ParsedNarrative | null>(null)
  const [showMenu, setShowMenu] = useState(false)
  const [worldState, setWorldState] = useState<WorldState>(() => createWorldStateManager().getState())
  ...
  const [activeLootContainer, setActiveLootContainer] = useState<LootContainer | null>(null)
  ...
  const addLog = useCallback((content: React.ReactNode, type: LogEntry["type"] = "narrative") => {
    setLogs((prev) => [
      ...prev,
      {
  ...
  const _addInteractiveLog = useCallback((narrative: ParsedNarrative, entities: EnvironmentalEntity[]) => {
    setCurrentNarrative(narrative)
    setGameState((prev) => ({
      ...prev,
      roomEnvironmentalEntities: [...prev.roomEnvironmentalEntities, ...entities],
    }))
  }, [])
  ...
  useEffect(() => {
    if (gameState.gameStarted && !gameState.gameOver && turnCountRef.current > 0) {
    ...
    (data: SaveData) => {
      ...
      setLogs(
        data.logs.map((log) => ({
          ...
          content: <span className="text-stone-400">{log.textContent}</span>,
        })),
      )
  ...
  const handleChangeStance = useCallback(
    (stance: CombatStance) => {
      ...
      const stanceNames = { balanced: "Balanced", aggressive: "Aggressive", defensive: "Defensive" }
      addLog(
        <span className="text-stone-400 text-sm">
          You shift to a <span className="text-amber-300">{stanceNames[stance]}</span> stance.
        </span>,
        "system",
      )
    },
  ...
  const handleEnvironmentalInteraction = useCallback(
    async (entityId: string, interactionId: string) => {
      ...
      const entity = gameState.roomEnvironmentalEntities.find((e) => e.id === entityId)
      if (!entity) {
        setIsProcessing(false)
        return
      }

      const interaction = entity.possibleInteractions.find((i) => i.id === interactionId)
      if (!interaction) {
        setIsProcessing(false)
        return
      }
      ...
      addLog(
        <span className="text-stone-400 italic">
          You {interaction.label.toLowerCase()} the <EntityText type="item">{entity.name}</EntityText>...
        </span>,
        "narrative",
      )
      ...
      if (result) {
        ...
        if (result.rewards) {
          if (result.rewards.gold && result.rewards.gold > 0) {
            setGameState((prev) => ({
              ...
              player: {
                ...prev.player,
                stats: { ...prev.player.stats, gold: prev.player.stats.gold + result.rewards!.gold! },
              },
            }))
            addLog(
              <span>
          ...
          if (result.rewards.healing && result.rewards.healing > 0) {
            setGameState((prev) => ({
              ...
              player: {
                ...
                stats: {
                  ...prev.player.stats,
                  health: Math.min(prev.player.stats.maxHealth, prev.player.stats.health + result.rewards!.healing!),
                },
              },
            }))
            addLog(
          ...
          if (result.rewards.damage && result.rewards.damage > 0) {
            setGameState((prev) => ({
              ...
              player: {
                ...
                stats: {
                  ...prev.player.stats,
                  health: Math.max(0, prev.player.stats.health - result.rewards!.damage!),
                },
              },
            }))
            addLog(
          ...
          if (result.rewards.experience && result.rewards.experience > 0) {
            setGameState((prev) => ({
              ...
              player: {
                ...
                stats: {
                  ...prev.player.stats,
                  experience: prev.player.stats.experience + result.rewards!.experience!,
                },
              },
            }))
          }
        ...
        if (result.consequences?.entityConsumed) {
          setGameState((prev) => ({
            ...prev,
            roomEnvironmentalEntities: prev.roomEnvironmentalEntities.map((e) =>
              e.id === entityId ? { ...e, consumed: true } : e,
            ),
          }))
        }
        ...
        if (interaction.consumesItem && itemUsed) {
          setGameState((prev) => ({
            ...
            player: {
              ...prev.player,
              inventory: prev.player.inventory.filter((i) => i.name !== itemUsed),
            },
          }))
          addLog(<span className="text-stone-500 text-sm">Used {itemUsed}.</span>, "system")
        }

        // Companion reaction
        if (result.companionReaction && gameState.player.party.active.length > 0) {
        ...
        if (result.newEntity) {
          ...
          setGameState((prev) => ({
            ...prev,
            roomEnvironmentalEntities: [...prev.roomEnvironmentalEntities, newEnvEntity],
          }))
          addLog(
            <span>
  ...
  const handleSelectPath = useCallback(
    async (path: PathOption) => {
      ...
      if ((path.danger === "dangerous" || path.danger === "unknown") && Math.random() < 0.4 && !newHazard) {
        newHazard = generateHazard(gameState.floor, dungeonTheme)
        addLog(
          <span>
            <EntityText type="curse">{newHazard.name}</EntityText> fills this area. {newHazard.description}
          </span>,
          "effect",
        )
      }
      ...
      if (path.roomType === "enemy" || (path.roomType === "mystery" && Math.random() < 0.5)) {
        ...
        const buffedEnemy = newHazard ? applyHazardToEnemy(enemy, newHazard) : enemy

        addLog(
          <span>
            A <EntityText type="enemy">{enemy.name}</EntityText> blocks your path!
          </span>,
          "combat",
        )
        ...
        const guaranteedRarity = isRare ? (Math.random() < 0.3 ? "epic" : "rare") : undefined

        // Generate container with AI
        const container = await generateLootContainer(
          gameState.floor,
          gameState.currentDungeon?.theme,
          guaranteedRarity as "common" | "uncommon" | "rare" | "epic" | "legendary" | undefined
        )

        if (container) {
          // Map "epic" rarity to "legendary" for EntityText display (EntityText doesn't support "epic")
          const displayRarity = container.rarity === "epic" ? "legendary" : container.rarity
          addLog(
            <span>
              You discover a mysterious{" "}
              <EntityText type={displayRarity}>{container.name}</EntityText>...
            </span>,
            "loot",
          )
          ...
          const loot = Math.random() < 0.5 ? generateWeapon(gameState.floor) : generateArmor(gameState.floor)
          const goldFound = Math.floor((Math.random() * 20 * gameState.floor + 10) * rewardMult)

          addLog(
            <span>
              You find a simple container with{" "}
              <EntityText type={loot.rarity}>{loot.name}</EntityText> and{" "}
          ...
          setGameState((prev) => ({
            ...
            player: {
              ...
              stats: { ...prev.player.stats, gold: prev.player.stats.gold + goldFound },
            },
            runStats: {
              ...prev.runStats,
        ...
        addLog(
          <span>
            You notice a <EntityText type="trap">{trap.name}</EntityText> ahead!
          </span>,
          "narrative",
        )
        setGameState((prev) => ({
        ...
        addLog(
          <span>
            You discover a <EntityText type="shrine">{shrine.name}</EntityText>.
          </span>,
          "narrative",
        )
        setGameState((prev) => ({
        ...
        addLog(
          <span>
            You encounter <EntityText type="npc">{npc.name}</EntityText>.
          </span>,
          "narrative",
        )
        setGameState((prev) => ({
        ...
        generateNPCDialogue(
          {
            name: npc.name,
            role: npc.role as "merchant" | "trapped" | "mysterious" | "quest_giver",
          ...
          },
          {
            ...
            playerHealth: Math.round((gameState.player.stats.health / gameState.player.stats.maxHealth) * 100),
          }
        ).then(aiDialogue => {
          if (aiDialogue) {
        ...
        addLog(
          <span className="text-stone-500">The path leads to an empty chamber. You may continue.</span>,
          "narrative",
        )
        setGameState((prev) => ({
          ...prev,
  ...
  const handleSelectClass = useCallback(
    (classId: PlayerClass) => {
      ...
      addLog(
        <span>
          You have chosen the path of the <EntityText type="rare">{classDef.name}</EntityText>. {classDef.lore}
        </span>,
        "system",
      )
      addLog(
  ...
  const selectDungeon = useCallback(
    async (dungeon: DungeonCard, keyToUse: DungeonKey) => {
      ...
      if (dungeon.isMystery) {
        addLog(
          <span>
            ...
            <EntityText
              type={keyToUse.rarity === "legendary" ? "legendary" : keyToUse.rarity === "rare" ? "rare" : "item"}
            >
              {keyToUse.name}
            </EntityText>{" "}
            to unlock the <EntityText type="legendary">???</EntityText> dungeon...
          </span>,
          "system",
        )
        addLog(
          <span className="text-purple-400 italic">
            The door swings open to reveal... <EntityText type="location">{dungeon.name}</EntityText>!
          </span>,
          "narrative",
        )
      } else {
      ...
      if (keyToUse.consumedOnUse) {
        addLog(
          <span className="text-muted-foreground text-sm">The key crumbles to dust as the lock clicks open.</span>,
          "system",
        )
      }
  ...
  const calculateDamage = useCallback((attacker: { attack: number }, defender: { defense: number }) => {
    ...
    setGameState((prev) => {
      ...
      while (player.stats.experience >= player.stats.experienceToLevel) {
        player = {
          ...
          resources: {
            ...player.resources,
            max: player.resources.max + (player.class ? CLASSES[player.class].resourcePerLevel : 0),
            current: player.resources.current + (player.class ? CLASSES[player.class].resourcePerLevel : 0),
          },
        }
        levelsGained++

        if (player.class) {
          const newAbilities = getClassAbilitiesForLevel(player.class, player.stats.level)
          const currentIds = new Set(player.abilities.map((a) => a.id))
          const unlocked = newAbilities.filter((a) => !currentIds.has(a.id))
          if (unlocked.length > 0) {
            player = { ...player, abilities: [...player.abilities, ...unlocked] }
            for (const ability of unlocked) {
              addLog(
                <span>
                  You learned a new ability: <EntityText type="rare">{ability.name}</EntityText>!
                </span>,
                "system",
              )
            }
      ...
      if (levelsGained > 0) {
        addLog(
          <span>
            ...
            <span className="text-amber-400">{player.stats.level}</span>!
          </span>,
          "system",
        )
      }
  ...
  const processTurnEffects = useCallback(() => {
    ...
    if (tickDamage > 0) {
      ...
      addLog(
        <span>
          You take <EntityText type="damage">{tickDamage}</EntityText> damage from status effects.
        </span>,
        "effect",
      )
      updateRunStats({ damageTaken: gameState.runStats.damageTaken + tickDamage })
    }
    if (tickHeal > 0) {
      newHealth = Math.min(player.stats.maxHealth, newHealth + tickHeal)
      addLog(
        <span>
          You recover <EntityText type="heal">{tickHeal}</EntityText> health from regeneration.
        </span>,
        "effect",
      )
    }
    ...
    for (const effect of expiredEffects) {
      addLog(
        <span>
          <EntityText type={effect.effectType === "buff" ? "blessing" : "curse"}>{effect.name}</EntityText> has worn
          off.
        </span>,
        "effect",
      )
      ...
      if (hazardDamage > 0) {
        ...
        addLog(
          <span>
            ...
            <EntityText type="damage">-{hazardDamage}</EntityText> HP. {hazardResult.narration}
          </span>,
          "effect",
        )
        updateRunStats({ damageTaken: gameState.runStats.damageTaken + hazardDamage })
      }
    }

    let updatedPlayer = {
      ...playerWithHazardEffects,
      stats: { ...playerWithHazardEffects.stats, health: Math.max(0, newHealth) },
    ...
    const updatedHazard = gameState.currentHazard ? tickHazard(gameState.currentHazard) : null

    // If hazard expired, remove its effects from the player
    if (gameState.currentHazard && !updatedHazard) {
      updatedPlayer = removeHazardEffects(updatedPlayer, gameState.currentHazard.id)
      addLog(
        <span>
          The <EntityText type="curse">{gameState.currentHazard.name}</EntityText> fades away.
        </span>,
        "effect",
      )
    }
  ...
  const handleUseAbility = useCallback(
    async (ability: Ability) => {
      ...
      if (!result.success) {
        addLog(<span className="text-red-400">{result.narration}</span>, "system")
        setIsProcessing(false)
        return
      }
      ...
      if (ability.damageType) {
        const comboResult = checkForCombo(updatedPlayer.combo, ability.damageType)
      ...
      if (ability.targetType === "self" && result.effectsApplied) {
        updatedPlayer = {
          ...updatedPlayer,
          activeEffects: [...updatedPlayer.activeEffects, ...result.effectsApplied],
        }
      }
      ...
      if (result.healing && result.healing > 0) {
        updatedPlayer = {
          ...
          stats: {
            ...updatedPlayer.stats,
            health: Math.min(updatedPlayer.stats.maxHealth, updatedPlayer.stats.health + result.healing),
          },
        }
        addLog(
          <span>
      ...
      if (result.damage && result.damage > 0) {
        ...
        updateRunStats({ damageDealt: gameState.runStats.damageDealt + finalDamage })
        ...
        addLog(
          <span>
            {result.narration} {isCrit && <span className="text-orange-400">CRITICAL! </span>}
            <EntityText type="enemy">{enemy.name}</EntityText> takes{" "}
            <EntityText type="damage">{finalDamage}</EntityText> damage!
            {effectivenessText && (
        ...
        if (result.effectsApplied && ability.targetType === "enemy") {
          for (const effect of result.effectsApplied) {
            addLog(
              <span>
                <EntityText type="enemy">{enemy.name}</EntityText> is afflicted with{" "}
                <EntityText type={effect.effectType === "buff" ? "blessing" : "curse"}>{effect.name}</EntityText>!
              </span>,
              "effect",
            )
          }
        ...
        if (ability.lifeSteal && ability.lifeSteal > 0) {
          ...
          updatedPlayer = {
            ...
            stats: {
              ...updatedPlayer.stats,
              health: Math.min(updatedPlayer.stats.maxHealth, updatedPlayer.stats.health + stolen),
            },
          }
          addLog(
            <span>
              You drain <EntityText type="heal">{stolen}</EntityText> life from your enemy.
            </span>,
            "combat",
          )
        }
        ...
        if (newEnemyHealth <= 0) {
          ...
          addLog(
            <span>
              <EntityText type="enemy">{enemy.name}</EntityText> has been slain! You gain{" "}
              <EntityText type="gold">{goldGain}</EntityText> gold and{" "}
              <span className="text-emerald-400">{expGain}</span> experience.
            </span>,
            "combat",
          )
          ...
          if (enemy.loot) {
            ...
            addLog(
              <span>
                You found: <EntityText type={enemy.loot.rarity}>{enemy.loot.name}</EntityText>!
              </span>,
              "loot",
            )
          }
        ...
        const tickedEnemy = tickEnemyAbilities({ ...enemy, health: newEnemyHealth })
  ...
  const triggerDeath = useCallback((causeOfDeath: string, killedBy?: string) => {
    setGameState((prev) => ({
      ...prev,
  ...
  const enemyAttack = useCallback(
    async (enemy: typeof gameState.currentEnemy, player: Player) => {
      ...
      if (Math.random() < player.stats.dodgeChance) {
        addLog(
          <span>
            <EntityText type="enemy">{enemy.name}</EntityText> attacks but you{" "}
            <span className="text-cyan-400">dodge</span> the blow!
          </span>,
          "combat",
        )
        setGameState((prev) => ({ ...prev, combatRound: prev.combatRound + 1 }))
        return
      }

      const selectedAbility = selectEnemyAbility(enemy, player.stats.health, player.stats.maxHealth)
      ...
      if (selectedAbility) {
        ...
        finalDamage = Math.max(
          1,
          baseDamage - Math.floor(effectiveStats.defense * 0.5 * STANCE_MODIFIERS[player.stance].defense),
        )
        _damageType = selectedAbility.damageType

        addLog(
          <span>
            <EntityText type="enemy">{enemy.name}</EntityText> uses{" "}
            <span className="text-red-400">{selectedAbility.name}</span>! {selectedAbility.narration}{" "}
        ...
        if (selectedAbility.effect) {
          addLog(
            <span>
              You are afflicted with <EntityText type="curse">{selectedAbility.effect.name}</EntityText>!
            </span>,
            "effect",
          )
        }
        ...
        const enemyDamage = Math.max(
          1,
          enemy.attack - Math.floor(effectiveStats.defense * 0.5 * STANCE_MODIFIERS[player.stance].defense),
        )
        const variance = Math.floor(Math.random() * 5) - 2
        ...
        addLog(
          <span>
            <EntityText type="enemy">{enemy.name}</EntityText> strikes back for{" "}
            <EntityText type="damage">{finalDamage}</EntityText> damage!
          </span>,
          "combat",
        )
  ...
  const processCompanionTurns = useCallback(
    async (
      enemy: Enemy,
      player: Player,
    ): Promise<{ enemy: Enemy | null; party: typeof player.party; playerHealed: number }> => {
      ...
      for (const companion of activeCompanions) {
        ...
        switch (action.action) {
          case "attack": {
            ...
            addLog(
              <span>
                ...
                <EntityText type="enemy">{currentEnemy.name}</EntityText> for{" "}
                <EntityText type="damage">{damage}</EntityText> damage!
              </span>,
              "combat",
            )
            ...
            if (newHealth <= 0) {
              ...
              addLog(
                <span>
                  <span className={colorClass}>{companion.name}</span> delivers the killing blow!
                </span>,
                "combat",
              )
          ...
          case "ability": {
            ...
            if (action.ability.effect.type === "damage" && currentEnemy) {
              ...
              if (newHealth <= 0) {
                currentEnemy = null
                updatedCompanion = modifyBond(companion, 5, "Killed enemy with ability")
              } else {
                currentEnemy = { ...currentEnemy, health: newHealth }
              }
              ...
              addLog(
                <span>
                  ...
                  <EntityText type="enemy">{currentEnemy.name}</EntityText>!{" "}
                  {action.ability.narration}
                </span>,
                "combat",
              )
            ...
            updatedCompanion = useCompanionAbility(updatedCompanion, action.ability.id)
            break
          }
          ...
          case "defend": {
            ...
            addLog(
              <span>
                <span className={colorClass}>{companion.name}</span> takes a defensive stance, protecting you!
              </span>,
              "combat",
            )
            break
          ...
          case "flee": {
            ...
            addLog(
              <span className="text-yellow-500">
                <span className={colorClass}>{companion.name}</span> panics and flees from battle!
              </span>,
              "combat",
            )
        ...
        updatedCompanion = { ...updatedCompanion, turnsWithPlayer: companion.turnsWithPlayer + 1 }

        // Update the companion in the party
        if (updatedParty) {
          updatedParty = {
            ...updatedParty,
            active: updatedParty.active.map((c) => (c.id === companion.id ? updatedCompanion : c)),
          }
        }
      }

      return { enemy: currentEnemy, party: updatedParty, playerHealed: totalPlayerHealed }
    },
    [addLog],
  )
  ...
  const playerAttack = useCallback(async () => {
    ...
    if (attackResponse) {
      ...
      addLog(
        <span>
          ...
          <EntityText type="enemy">{gameState.currentEnemy.name}</EntityText> for{" "}
          <EntityText type="damage">{damage}</EntityText> damage.
          {effectivenessNote && (
            <span className={effectiveness === "effective" ? "text-emerald-400" : "text-stone-500"}>
              {effectivenessNote}
    ...
    if (newEnemyHealth <= 0) {
      ...
      if (victoryResponse) {
        ...
        addLog(
          <span>
            The <EntityText type="enemy">{gameState.currentEnemy.name}</EntityText> falls! You gain{" "}
      ...
      if (gameState.currentEnemy.lastWords) {
        addLog(
          <span className="italic text-muted-foreground">&quot;{gameState.currentEnemy.lastWords}&quot;</span>,
          "narrative",
        )
      }

      // Check if this was a boss - generate special rewards
      const isBoss = gameState.currentEnemy.name.includes("Lord") ||
        ...
        getBossVictoryRewards(
          ...
          if (reward) {
            ...
            reward.items.forEach(item => {
              addLog(
                <span>
                  Boss Trophy: <EntityText type={item.rarity}>{item.name}</EntityText>
                </span>,
                "loot"
              )
            })
      ...
      if (!companionResult.enemy) {
        ...
        const expGain = Math.floor(tickedEnemy.expReward * calculateEffectiveStats(updatedPlayer).expMultiplier)
        const goldGain = Math.floor(tickedEnemy.goldReward * calculateEffectiveStats(updatedPlayer).goldMultiplier)
  ...
  const exploreRoom = useCallback(async () => {
    ...
    if (playerDied) {
      ...
      addLog(<span className="text-red-400">The poison claims your life. Darkness takes you.</span>, "system")
      setIsProcessing(false)
      return
    }
    ...
    async (action: "disarm" | "trigger" | "avoid") => {
      ...
      if (action === "disarm") {
        ...
        if (success) {
          addLog(
            <span>
              You carefully disarm the <EntityText type="trap">{trap.name}</EntityText>.
          ...
          setGameState((prev) => ({
            ...
            player: {
              ...prev.player,
              stats: { ...prev.player.stats, experience: prev.player.stats.experience + 5 },
            },
            phase: "dungeon",
            activeTrap: null,
          }))
          ...
          addLog(
            <span>
              Your disarm attempt fails! The <EntityText type="trap">{trap.name}</EntityText> activates!{" "}
          ...
          if (trap.effect) {
            ...
            addLog(
              <span>
                You are afflicted with <EntityText type="curse">{trap.effect.name}</EntityText>!
              </span>,
              "effect",
            )
          }
          ...
          if (newHealth <= 0) {
            ...
            addLog(<span className="text-red-400">The trap proves fatal. Darkness claims you.</span>, "system")
          } else {
            setGameState((prev) => ({
              ...prev,
              player: {
        ...
        addLog(
          <span>
            You deliberately trigger the <EntityText type="trap">{trap.name}</EntityText>.{" "}
            <EntityText type="damage">-{damage} HP</EntityText>
          </span>,
          "combat",
        )
        ...
        if (avoided) {
          addLog(
            <span>
              You carefully edge past the <EntityText type="trap">{trap.name}</EntityText>.
            </span>,
            "narrative",
          )
          setGameState((prev) => ({
          ...
          addLog(
            <span>
              You fail to avoid the <EntityText type="trap">{trap.name}</EntityText>!{" "}
              <EntityText type="damage">-{damage} HP</EntityText>
            </span>,
            "combat",
          )
  ...
  const handleShrineAction = useCallback(
    async (action: "accept" | "decline" | "desecrate") => {
      ...
      if (action === "decline") {
        addLog(
          <span className="text-muted-foreground">
            You leave the <EntityText type="shrine">{shrine.name}</EntityText> undisturbed.
          </span>,
          "narrative",
        )
        setGameState((prev) => ({ ...prev, phase: "dungeon", activeShrine: null }))
        setIsProcessing(false)
        return
      }
      ...
      if (action === "desecrate" && shrine.shrineType === "dark") {
        ...
        if (roll < 0.3) {
          ...
          addLog(
            <span>
              You desecrate the <EntityText type="shrine">{shrine.name}</EntityText>. Dark power floods through you!{" "}
          ...
          setGameState((prev) => ({
            ...prev,
            player: { ...prev.player, activeEffects: [...prev.player.activeEffects, effect] },
            phase: "dungeon",
            activeShrine: null,
          }))
        } else if (roll < 0.7) {
          ...
          addLog(
            <span>
              The shrine&apos;s dark power lashes out! <EntityText type="curse">{curse.name}</EntityText> afflicts you!
            </span>,
            "effect",
          )
          setGameState((prev) => ({
            ...prev,
            player: { ...prev.player, activeEffects: [...prev.player.activeEffects, curse] },
            phase: "dungeon",
            activeShrine: null,
          }))
        } else {
          ...
          addLog(
            <span>
              The shrine explodes with malevolent energy! <EntityText type="damage">-{damage} HP</EntityText>
            </span>,
            "combat",
          )
          const newHealth = gameState.player.stats.health - damage
      ...
      let canAfford = true
      if (shrine.cost?.gold && gameState.player.stats.gold < shrine.cost.gold) canAfford = false
      if (shrine.cost?.health && gameState.player.stats.health <= shrine.cost.health) canAfford = false

      if (!canAfford) {
        addLog(<span className="text-muted-foreground">You cannot afford this offering.</span>, "system")
        setGameState((prev) => ({ ...prev, phase: "dungeon", activeShrine: null }))
        setIsProcessing(false)
        return
      }
      ...
      if (shrine.cost?.gold) {
        ...
        updateRunStats({ goldSpent: gameState.runStats.goldSpent + shrine.cost.gold })
        addLog(
          <span>
            You offer <EntityText type="gold">{shrine.cost.gold} gold</EntityText> to the shrine.
          </span>,
          "narrative",
        )
      }
      if (shrine.cost?.health) {
        ...
        addLog(
          <span>
            You sacrifice <EntityText type="damage">{shrine.cost.health} HP</EntityText> to the shrine.
          </span>,
          "narrative",
        )
      }
      ...
      switch (shrine.shrineType) {
        ...
        case "fortune":
          ...
          break
        case "unknown":
          const effects = [STATUS_EFFECTS.blessed(), STATUS_EFFECTS.fortified(), STATUS_EFFECTS.regeneration()]
      ...
      if (effect) {
        addLog(
          <span>
            The shrine bestows <EntityText type="blessing">{effect.name}</EntityText> upon you!
          </span>,
          "effect",
        )
      }
      ...
      setGameState((prev) => ({
        ...
        player: {
          ...
          activeEffects: effect ? [...prev.player.activeEffects, effect] : prev.player.activeEffects,
        },
        phase: "dungeon",
        activeShrine: null,
      }))
  ...
  const handleNPCChoice = useCallback(
    async (optionId: string) => {
      ...
      if (optionId === "leave") {
        addLog(
          <span className="text-muted-foreground">
            You nod to <EntityText type="npc">{npc.name}</EntityText> and continue on your way.
          </span>,
          "narrative",
        )
        setGameState((prev) => ({ ...prev, phase: "dungeon", activeNPC: null }))
        setIsProcessing(false)
        return
      }
      ...
      if (optionId === "trade" && npc.role === "merchant" && npc.inventory?.length) {
        ...
        if (gameState.player.stats.gold >= cost) {
          ...
          addLog(
            <span>
              You purchase <EntityText type={item.rarity}>{item.name}</EntityText> for{" "}
          ...
          addLog(<span className="text-muted-foreground">You don&apos;t have enough gold.</span>, "system")
        }
        setIsProcessing(false)
        return
      }
      ...
      if (optionId === "help" && npc.role === "trapped") {
        ...
        if (roll < 0.3) {
          addLog(
            <span>
              ...
              <EntityText type="companion">&quot;I&apos;ll remember this kindness!&quot;</EntityText>
            </span>,
            "dialogue",
          )
        }
        const goldReward = Math.floor(Math.random() * 30) + 20
        updateRunStats({ goldEarned: gameState.runStats.goldEarned + goldReward })
        addLog(
          <span>
        ...
        setGameState((prev) => ({
          ...prev,
          player: { ...prev.player, stats: { ...prev.player.stats, gold: prev.player.stats.gold + goldReward } },
          phase: "dungeon",
          activeNPC: null,
        }))
        setIsProcessing(false)
      ...
      if (optionId === "talk") {
        const generateEntity = async (entityType: string, _options: unknown) => {
          if (entityType === "npc") {
            return {
        ...
        const dialogue = await generateEntity("npc", { role: npc.role, floor: gameState.floor })
        const newDialogue = dialogue?.greeting || "The dungeon holds many secrets..."
        setNpcDialogue(newDialogue)
        addLog(<span className="italic text-amber-200/80">&quot;{newDialogue}&quot;</span>, "dialogue")
        setIsProcessing(false)
        return
      }
      ...
      if (optionId === "attack") {
        ...
        addLog(
          <span>
            <EntityText type="npc">{npc.name}</EntityText> cries out as you attack!{" "}
  ...
  const handleLootContainerComplete = useCallback((items: Item[], curseTriggered?: boolean, curseEffect?: string) => {
    const goldFound = items.reduce((sum, item) => sum + (item.value || 0), 0)

    // Log collected items
    addLog(
      <span>
        Collected {items.length} item{items.length !== 1 ? "s" : ""}: {items.map((item, i) => (
          <span key={item.id}>
            {i > 0 && ", "}
            <EntityText type={item.rarity}>{item.name}</EntityText>
          </span>
        ))}
        {goldFound > 0 && <> worth <EntityText type="gold">{goldFound} gold</EntityText></>}
      </span>,
      "loot",
    )
    ...
    if (curseTriggered && curseEffect) {
      ...
      setGameState((prev) => ({
        ...
        player: {
          ...prev.player,
          stats: { ...prev.player.stats, health: Math.max(1, prev.player.stats.health - curseDamage) },
        },
      }))
    }
  ...
  const handleLootContainerCancel = useCallback(() => {
    addLog(
      <span className="text-muted-foreground italic">You decide to leave the container unopened...</span>,
      "narrative",
    )
    setActiveLootContainer(null)
  }, [addLog])
  ...
  const descendFloor = useCallback(async () => {
    ...
    if (newFloor > maxFloors) {
      const dungeonRarity: ItemRarity = gameState.currentDungeon?.rarity || "common"
      ...
      updateRunStats({
        dungeonsCompleted: [...gameState.runStats.dungeonsCompleted, gameState.currentDungeon?.name || "Unknown"],
      ...
      setTimeout(() => {
        setGameState((prev) => ({
          ...
          player: {
            ...prev.player,
            stats: { ...prev.player.stats, gold: prev.player.stats.gold + bonusGold },
            keys: rewardKey ? [...prev.player.keys, rewardKey] : prev.player.keys,
      ...
      updateRunStats({ floorsCleared: gameState.runStats.floorsCleared + gameState.currentRoom })
  ...
  const attemptFlee = useCallback(async () => {
    ...
    if (gameState.currentHazard?.effects.fleeDisabled) {
      addLog(
        <span className="text-red-400">
          The <EntityText type="curse">{gameState.currentHazard.name}</EntityText> prevents your escape!
        </span>,
        "system",
      )
      return
    ...
    if (success) {
      ...
      setGameState((prev) => ({ ...prev, inCombat: false, currentEnemy: null, combatRound: 1 }))
    } else {
      const effectiveStats = calculateEffectiveStats(gameState.player)
      const damage = calculateDamage(gameState.currentEnemy, { defense: effectiveStats.defense })
      const newHealth = gameState.player.stats.health - damage
      ...
      if (fleeResponse) {
        addLog(
          <span>
            {fleeResponse.fleeNarration} <EntityText type="damage">(-{damage})</EntityText>
          </span>,
          "combat",
        )
      } else {
        addLog(
          <span>
            Failed to flee! The <EntityText type="enemy">{gameState.currentEnemy.name}</EntityText> strikes for{" "}
            <EntityText type="damage">{damage}</EntityText> damage.
          </span>,
          "combat",
        )
      ...
      if (newHealth <= 0) {
        ...
        addLog(
          <span className="text-red-400">
            <EntityText type="player">You</EntityText> have fallen. The dungeon claims another soul.
          </span>,
          "system",
        )
      } else {
    ...
    setIsProcessing(false)
  }, [gameState, isProcessing, calculateDamage, addLog, updateRunStats, triggerDeath])

  const consumePotion = useCallback(
    (potion: Item) => {
      if (potion.type !== "potion" || !potion.stats?.health) return

      const healAmount = Math.min(potion.stats.health, gameState.player.stats.maxHealth - gameState.player.stats.health)

      addLog(
        <span>
          <EntityText type="player">You</EntityText> consume <EntityText type="potion">{potion.name}</EntityText>.{" "}
      ...
      setGameState((prev) => ({
        ...
        player: {
          ...prev.player,
          stats: { ...prev.player.stats, health: prev.player.stats.health + healAmount },
    ...
    (item: Item) => {
      ...
      addLog(
        <span>
          ...
          <EntityText type={item.rarity === "legendary" ? "legendary" : item.rarity === "rare" ? "rare" : "item"}>
            {item.name}
          </EntityText>
          .
        </span>,
      ...
      setGameState((prev) => ({
        ...
        player: {
          ...prev.player,
          equipment: { ...prev.player.equipment, [slot]: { ...item, equipped: true } },
          inventory: [
            ...prev.player.inventory.filter((i) => i.id !== item.id),
            ...(currentEquipped ? [{ ...currentEquipped, equipped: false }] : []),
          ],
        },
      }))
    },
  ...
  const _returnToTavern = useCallback(() => {
          ...
          ? CLASSES[prev.player.class].startingAbilities
              .map((id) => {
                ...
                const _allAbilities = Object.values(CLASSES).flatMap((c) =>
                  c.abilityUnlocks.map((u) => u.abilityId).concat(c.startingAbilities),
                )
                return prev.player.abilities.find((a) => a.id === id) || prev.player.abilities[0]
              })
              .filter(Boolean)
          : [],
    ...
    addLog(
      <span className="text-stone-400">You awaken in the tavern. The nightmare fades, but the dungeons await...</span>,
      "system",
    )
  ...
  const startGame = useCallback(() => {
    ...
    setAiNarration({})
    setShowClassSelect(true)
    addLog(<span>You stand at the entrance to the dungeon depths. Choose your path wisely...</span>, "narrative")
  }, [addLog])
      ...
      setGameState((prev) => ({
        ...
        player: {
          ...
          stats: {
            ...
            health: Math.min(prev.player.stats.maxHealth, prev.player.stats.health + amount),
          },
        },
      }))
      addLog(
        <span>
          Sister Meridia&apos;s healing light washes over you. <EntityText type="heal">+{amount} HP</EntityText>
        </span>,
        "system",
      )
    },
  ...
  const handleBuyKey = useCallback(
    (keyRarity: "common" | "uncommon" | "rare") => {
      ...
      addLog(
        <span>
          You purchase a <EntityText type={keyRarity === "rare" ? "rare" : "item"}>{key.name}</EntityText> from Korvin.
        </span>,
        "system",
      )
    },
  ...
  const currentChoices = useMemo(() => {
    ...
    if (gameState.inCombat && gameState.currentEnemy) {
      const choices: GameChoice[] = [
        ...
        {
          id: "flee",
          text: gameState.currentHazard?.effects.fleeDisabled ? "Flee (Blocked)" : "Flee",
          action: attemptFlee,
          ...
          disabled: isProcessing || gameState.currentHazard?.effects.fleeDisabled,
        },
      ]

      if (potion && gameState.player.stats.health < gameState.player.stats.maxHealth) {
        choices.push({
          id: "potion",
    ...
    if (!gameState.inCombat && gameState.currentDungeon) {
      ...
      if (gameState.currentRoom > 0 && gameState.currentRoom % 5 === 0) {
        ...
        explorationChoices.push({
          id: "descend",
          text: isLastFloor ? "Claim Victory & Exit" : `Descend to Floor ${gameState.floor + 1}`,
          action: descendFloor,
          disabled: isProcessing,
        })
      }
      ...
      if (currentItem) {
        ...
        setGameState((prev) => ({
          ...
          player: {
            ...
            inventory: [...prev.player.inventory, { ...currentItem, equipped: false }],
          },
        }))
      }
    },
  ...
  const renderInteractiveEntities = useCallback(() => {
    const activeEntities = gameState.roomEnvironmentalEntities.filter((e) => !e.consumed)
    if (activeEntities.length === 0) return null

    return (
      <div className="mt-2 space-y-1">
        <span className="text-xs text-muted-foreground uppercase tracking-wider">Nearby:</span>
        <div className="flex flex-wrap gap-2">
          {activeEntities.map((entity) => {
            const interactions = getAvailableInteractions(entity, gameState.player)
            ...
            return (
              <button
                ...
                onClick={() => {
                  ...
                  if (available) {
                    handleEnvironmentalInteraction(entity.id, available.interaction.id)
                  }
                }}
  ...
  }, [gameState.roomEnvironmentalEntities, gameState.player, handleEnvironmentalInteraction, isProcessing])
      ...
      <div className="min-h-screen bg-stone-950 flex items-center justify-center">
        <div className="text-center space-y-8">
          <div className="space-y-2">
            <h1 className="text-4xl font-bold text-amber-100 tracking-wider">DEPTHS OF SHADOW</h1>
            <p className="text-stone-500 text-sm tracking-widest uppercase">A Text-Based Dungeon Crawler</p>
          </div>
          ...
          <p className="text-stone-600 text-xs pt-8">Press any key or click to begin your descent...</p>
        </div>
        ...
        {showClassSelect ? (
          ...
          <div className="flex-1 flex flex-col p-4">
            ...
            {!gameState.inCombat && gameState.currentHazard && (
              <div className="mb-3">
                <HazardDisplay
                  ...
                  isMitigated={gameState.currentHazard.mitigatedBy?.includes(gameState.player.class || "")}
                />
              </div>
            )}
            ...
            {gameState.pathOptions && gameState.pathOptions.length > 0 && (
              <PathSelect paths={gameState.pathOptions} onSelectPath={handleSelectPath} disabled={isProcessing} />
            )}
            ...
            {isProcessing && (
              <div className="text-center text-stone-500 text-sm mt-2 animate-pulse">The dungeon stirs...</div>
            )}
          </div>
        )}
      </div>
