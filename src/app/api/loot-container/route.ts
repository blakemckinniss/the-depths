...
const OpeningResultSchema = z.object({
  openingNarrative: z
    .string()
    .describe("Dramatic description of opening moment"),
  revealMoment: z
    .string()
  ...
  jackpotMoment: z
    .string()
    .nullish()
    .describe("Special text if legendary/jackpot"),
...
async function handleGenerate(
  ...
  // Pity system: increase rare chance after many common opens (using centralized ledger)
  const rarityBoost = calculatePityBonus(opensSinceRare || 0);

  // Base rarity distribution with floor scaling (using centralized ledger)
  const floorBonus = calculateFloorBonus(floor);
