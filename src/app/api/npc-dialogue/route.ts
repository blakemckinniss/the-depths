...
const DialogueResponseSchema = z.object({
  ...
  secrets: z
    .array(z.string())
    .nullish()
    .describe("Information NPC might reveal if trust is high"),
  questHook: z
    .object({
      available: z.boolean(),
    ...
    .nullish()
    .describe("Whether NPC has a quest to offer"),
});
...
const RequestSchema = z.object({
  ...
  context: z.object({
    ...
    recentEvents: z
      .array(z.string())
      .optional()
      .describe("Recent things that happened"),
  }),
...
export async function POST(request: Request) {
  try {
    ...
    // Cache key includes disposition range for variety (using centralized ledger)
    const dispositionRange = getDispositionLabel(npc.disposition);
...
${context.playerHealth !== undefined ? `- Player appears ${getHealthDescriptor(context.playerHealth)}` : ""}
